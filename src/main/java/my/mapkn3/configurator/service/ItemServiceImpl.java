...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemEntity> getAllItems() {
        ...
        log.debug(String.format("Get %d items:%n", items.size()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    }
    ...
    public List<ItemEntity> getAllByType(TypeEntity type) {
        ...
        log.debug(String.format("Get %d items for type:%n%s%n", items.size(), type.toString()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    ...
    public List<ItemEntity> getAllByFactory(FactoryEntity factory) {
        ...
        log.debug(String.format("Get %d items for factory:%n%s%n", items.size(), factory.toString()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    ...
    public List<ItemEntity> getAllBySeries(SeriesEntity series) {
        ...
        log.debug(String.format("Get %d items for series:%n%s%n", items.size(), series.toString()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    ...
    public List<ItemEntity> getAllByGroup(GroupEntity group) {
        ...
        log.debug(String.format("Get %d items for group:%n%s%n", items.size(), group.toString()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    ...
    public List<ItemEntity> getAllByFactoryAndSeriesAndGroup(FactoryEntity factory, SeriesEntity series, GroupEntity group) {
        ...
        log.debug(String.format("Get %d items for factory, series and group:%n%s%n%s%n%s%n", items.size(),
    ...
    public List<ItemEntity> getAllByFactoryAndSeries(FactoryEntity factory, SeriesEntity series) {
        ...
        log.debug(String.format("Get %d items for factory and series:%n%s%n%s%n", items.size(), factory.toString(), series.toString()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    ...
    public List<ItemEntity> getAllByFactoryAndGroup(FactoryEntity factory, GroupEntity group) {
        ...
        log.debug(String.format("Get %d items for factory and group:%n%s%n%s%n", items.size(), factory.toString(), group.toString()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    ...
    public List<ItemEntity> getAllBySeriesAndGroup(SeriesEntity series, GroupEntity group) {
        ...
        log.debug(String.format("Get %d items for series and group:%n%s%n%s%n", items.size(), series.toString(), group.toString()));
        for (ItemEntity item : items) {
            log.debug(String.format("%s%n", item.toString()));
        }
        return items;
    ...
    public ItemEntity getById(long id) {
        log.debug(String.format("Getting item with id = %d", id));
        ItemEntity item = itemRepository.findById(id).orElse(null);
        if (item == null) {
            log.debug(String.format("Item with id = %d not found", id));
        }
        return item;
    }
    ...
    public ItemEntity getByModel(String model) {
        log.debug(String.format("Getting item with model = %s", model));
        ItemEntity item = itemRepository.findByModel(model).orElse(null);
        if (item == null) {
            log.debug(String.format("Item with model = %s not found", model));
        }
        return item;
    }
    ...
    public ItemEntity getByOurArticle(String ourArticle) {
        log.debug(String.format("Getting item with our article = %s", ourArticle));
        ItemEntity item = itemRepository.findByOurArticle(ourArticle).orElse(null);
        if (item == null) {
            log.debug(String.format("Item with our article = %s not found", ourArticle));
        }
        return item;
    }
    ...
    public ItemEntity getByFactoryArticle(String factoryArticle) {
        log.debug(String.format("Getting item with factory article = %s", factoryArticle));
        ItemEntity item = itemRepository.findByFactoryArticle(factoryArticle).orElse(null);
        if (item == null) {
            log.debug(String.format("Item with factory article = %s not found", factoryArticle));
        }
        return item;
    }
    ...
    public ItemEntity addItem(ItemEntity item) {
        ...
        if (entity != null) {
            log.debug(String.format("Item already exist:%n%s", entity.toString()));
            return null;
        } else {
            log.debug(String.format("Add new item:%n%s", item.toString()));
            ItemEntity newItem = itemRepository.saveAndFlush(item);
            log.debug(String.format("Id for new item: %d", newItem.getId()));
            return newItem;
        }
    }
    ...
    public ItemEntity updateItem(ItemEntity item) {
        ...
        log.debug(String.format("Updated item:%n%s", updatedItem.toString()));
        return updatedItem;
    }

    @Override
    public void deleteItem(ItemEntity item) {
        log.debug(String.format("Deleted item:%n%s", item.toString()));
