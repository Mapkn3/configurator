...
public class FactoryServiceImpl implements FactoryService {
    ...
    public List<FactoryEntity> getAllFactories() {
        ...
        log.debug(String.format("Get %d factories:%n", factories.size()));
        for (FactoryEntity factory : factories) {
            log.debug(String.format("%s%n", factory.toString()));
        }
        return factories;
    }
    ...
    public FactoryEntity getFactoryById(long id) {
        log.debug(String.format("Getting factory with id = %d", id));
        FactoryEntity factory = factoryRepository.findById(id).orElse(null);
        if (factory == null) {
            log.debug(String.format("Factory with id = %d not found", id));
        }
        return factory;
    }
    ...
    public FactoryEntity getFactoryByName(String name) {
        log.debug(String.format("Getting factory with name = %s", name));
        FactoryEntity factory = factoryRepository.findByName(name).orElse(null);
        if (factory == null) {
            log.debug(String.format("Factory with name = %s not found", name));
        }
        return factory;
    }
    ...
    public FactoryEntity addFactory(FactoryEntity factory) {
        ...
        if (entity != null) {
            log.debug(String.format("Factory already exist:%n%s", entity.toString()));
            return null;
        } else {
            log.debug(String.format("Add new factory:%n%s", factory.toString()));
            FactoryEntity newFactory = factoryRepository.saveAndFlush(factory);
            log.debug(String.format("id for new factory: %d", newFactory.getId()));
            return newFactory;
        }
    }
    ...
    public FactoryEntity updateFactory(FactoryEntity factory) {
        ...
        log.debug(String.format("Updated factory:%n%s", updatedFactory.toString()));
        return updatedFactory;
    }

    @Override
    public void deleteFactory(FactoryEntity factory) {
        log.debug(String.format("Deleted factory:%n%s", factory.toString()));
